...
public class ItemSlot : PV_Object
{
    ...
    public void ClearMe()
    {
        LogHistoric( $"ClearMe()", PV_LogFormatting.UserMethod );
    ...
    {
        LogHistoric($"MainButtonAction()", PV_LogFormatting.UserMethod);
    ...
    {
        LogHistoric( $"'{name}'.UseMyItem(). currently holding: '{MyItemData.itemName}'", PV_LogFormatting.UserMethod );
        if( MyItemData == null || MyItemData.itemName == ItemName.empty )
        {
    ...
    {
        LogHistoric( $"Pickup('{pickupObject_passed.MyItemData.itemName}'). NumberHeld: '{NumberHeld}', requested pickup amount: '{pickupObject_passed.NumberHeld}'," +
            $"slot item data null?: '{MyItemData == null}', pickup item data null?: '{pickupObject_passed.MyItemData == null}'", PV_LogFormatting.Standard );
        ...
        {
            LogHistoric("had more than enough room to pickup this amount");
            amt += pickupObject_passed.NumberHeld ;
        }
        else
        {
            amt += ( pickupObject_passed.MyItemData.count_maxPerSlot - NumberHeld );
            LogHistoric($"only had enough room to pickup '{amt}', had to leave some behind.");

        }
        LogHistoric($"final caluclated amount to pick up: '{amt}'");
        NumberHeld += amt;
