...
public class PV_Pool : PV_Object
{
    ...
    public void Init()
    {
        LogHistoric( "Start() begin", PV_Enums.PV_LogFormatting.UnityAPIMethod );
        LogHistoric( $"count_allowedActiveInScene: '{count_allowedActiveInScene}'" );

        if ( poolPrefab != null && count_allowedActiveInScene > 0 )
        {
            LogHistoric("conditions met for overriding pool based on supplied prefab. Creating pool...");
        ...
        {
            LogHistoric( "pooledObjects list was set propertly. Setting all objects inactive..." );
            foreach ( GameObject g in pooledObjects )
            {
                g.SetActive( false );
            }
        }
        LogHistoric( "Start() end", PV_Enums.PV_LogFormatting.UnityAPIMethod );
    }

    public GameObject GetNext()
    {
        LogHistoric( $"CycleNext(), index_lastMadeActive: '{index_lastMadeActive}'" );
    ...
    {
        LogHistoric( $"[overload]CycleNext(pos_passed: '{pos_passed}', rot_passed: '{rot_passed}'), index_lastMadeActive: '{index_lastMadeActive}'" );
