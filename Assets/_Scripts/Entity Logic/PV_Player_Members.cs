...
public class PV_Player_Members : Base_living
{
    ...
    {
        LogHistoric( $"Heal('{amt}'). current hp: '{hp}'", PV_LogFormatting.UserMethod );
        hp += amt;
        if( hp > 100 )
        {
            hp = 100;
    ...
    {
        LogHistoric( $"{gameObject.name}.InitiateReload()", PV_LogFormatting.UserMethod );
    ...
    {
        LogHistoric( $"{gameObject.name}.SetReloaded()", PV_LogFormatting.UserMethod );
    ...
    {
        LogHistoric($"{nameof(AttemptPickup)}", PV_LogFormatting.UserMethod);
        ...
        if ( PV_GameManager.Instance.InventoryScript.CanPickup(pickupObject_passed) )
        {
            ...
            {
                LogHistoric("updating ammunitiondisplay...");
    ...
    {
        PV_Debug.Log($"EquipWeapon('{gun_passed}')", PV_LogFormatting.UserMethod, PV_LogDestination.Console);
        if( GunScript_equipped == gun_passed )
        {
            Debug.LogWarning($"EquipWeapon() was passed a gun ('{gun_passed}' that appears to already be equipped! Returning early...)");
            return;
        }
