...
public class Base_enemy : base_NPC
{
	...
	public virtual void CalculatePlayerVisibility(float timeMult_passed ) //public so that this can be called from the manager script a certain amount per second.
    {
            ...
            !Physics.Linecast(Trans_visionOrigin.position, PlayerFocusPosition, PV_Environment.Instance.Mask_CompletelyOpaque) )
		{
			...
			if ( !canSeePlayer_prev )
			{
				...
				LogHistoric( "Just saw player. Updated path", PV_LogFormatting.Standard );
			}
    ...
    */
    ...
    {
        LogHistoric($"EmulateEnvironmentalSound(pos_passed: '{pos_passed}', soundProjectDistance_passed: '{soundProjectDistance_passed}')", PV_LogFormatting.UserMethod);
		...
		if ( distToSoundOrigin <= (soundProjectDistance_passed * MyBaseStats.HearingMultiplier) )
		{
			...
			if( dist_soundHeard_calculated <= (soundProjectDistance_passed * MyBaseStats.HearingMultiplier) )
			{
				...
				LogHistoric($"decided that the sound was indeed heard.");
    ...
    {
        LogHistoric( $"GenerateNewPath()", PV_LogFormatting.UserMethod );
        int randomIndex = Random.Range(0, patrolPoints_cached.Count);
        LogHistoric($"generated randomindex: '{randomIndex}' out of '{patrolPoints_cached.Count}' cached points. This point corresponds to position: '{patrolPoints_cached[randomIndex].V_center}' with raduis: '{patrolPoints_cached[randomIndex].Radius}'");
        if ( randomIndex == index_currentlySelectedPatrolPoint )
        {
            ...
            LogHistoric($"decided against this index. looped randomindex to: '{randomIndex}'");
        }
        LogHistoric($"original index: '{index_currentlySelectedPatrolPoint}', updating to: '{randomIndex}'...");
        ...
        LogHistoric($"GenerateNewPath() end. index_currentlySelectedPatrolPoint: '{index_currentlySelectedPatrolPoint}', cd_PatrolWait: '{cd_PatrolWait}'", PV_LogFormatting.UserMethod);
    }

    public virtual void UpdatePath(Vector3 v_passed, float sampleMaxDistance)
    {
        LogHistoric($"UpdatePath()", PV_LogFormatting.UserMethod);
        ...
        else
        {
            ...
            LogHistoric($"airspace manager found '{pointsReturned.Count}' points. using these to calculate a PV_Path...");
	...
	{
		LogHistoric( $"{nameof(EndOfAttackAnimationCleanup)}()", PV_LogFormatting.UserMethod );
    ...
    {
        LogHistoric("makedead()", PV_LogFormatting.UserMethod);
