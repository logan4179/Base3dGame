...
public class Enemy_Bug : Base_Enemy_terrestrial
{
	...
	void FixedUpdate()
	{
		...
		if (myPatrolMode != PatrolMode.Stationary && myActionState == EnemyActionState.Patrolling)
		{
			if (cd_PatrolWait <= 0f)
			{
				...
				{
					LogHistoric($"Bug enemy has reached end goal, and is negotiating a patrol wait state...");
					GenerateNewPath();
					LogHistoric($"Bug enemy: '{name}', settled on a patrol wait of '{cd_PatrolWait}', and an end goal of: '{MyPath.EndGoal}'...");
	...
	{
		LogHistoric($"SwitchState({state_passed})", PV_LogFormatting.UserMethod);
		...
		LogHistoric($"Bug enemy: '{name}' state switched to '{myActionState}' from previous state of: '{prevState}'");
	}
		...
		if (!AmTotallyPreoccupied && myActionState == EnemyActionState.Engaging)
		{
			...
			if (cd_ApproachingPlayerBeforeStriking <= 0f)
			{
				...
				if (myNextMove <= (MyStats_Base_Enemy.AggressionLevel + likelihood_attack_calculated))
				{
					...
					{
						LogHistoric($"Landed on charge attack!");
						...
						LogHistoric($"{name} Initiating attack...", PV_LogFormatting.Standard);
					}
					else //if (attackChoice <= (MyStats.Attack_basicStrike.Likelihood * (1f / dist_toPlayer)))
					{
						LogHistoric($"Landed on regular attack!");
	...
	public void Attack()
	{
		LogHistoric("Attack()", PV_LogFormatting.UserMethod);
	...
	{
		LogHistoric($"ActivateMeViaArea('{name}')");
	...
	public override void DeactivateMeViaArea()
	{
		...
		MGR_BugEnemy.Instance.UnregisterAndDeactivateBug(this);
	}
