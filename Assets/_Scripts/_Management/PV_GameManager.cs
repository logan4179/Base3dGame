...
public class PV_GameManager : MonoBehaviour
{
    ...
    public static float GravityBuildRate = 1f;
    ...
    public static PromptEvent Event_OnPromptEntered, Event_OnPromptReadNext, Event_OnPromptExited;

    //public static PickupEvent Event_OnPickup; //trying taking this away because A) it's a nightmare to trace, and B) I think a lot of the stuff I'm using this for actually makes sense in the following event, which assumes the item has already been picked up
	...
	private void OnEnable()
    {
        ...
        Event_OnPromptEntered = new PromptEvent();
        Event_OnPromptReadNext = new PromptEvent();
        Event_OnPromptExited = new PromptEvent();
    ...
    private void OnDisable()
    {
        ...
        Event_OnPromptEntered.RemoveAllListeners();
        Event_OnPromptReadNext.RemoveAllListeners();
        Event_OnPromptExited.RemoveAllListeners();
    ...
    void Start()
    {
        ...
        Event_OnPromptEntered.AddListener(OnPromptEntered_action);
        Event_OnPromptReadNext.AddListener(OnPromptReadNext_action);
        Event_OnPromptExited.AddListener(OnPromptExited_action);
    ...
    {
        print($"OnPromptReadNext_action, object: '{promptOb_passed.gameObject.name}'");

    }

    void OnPromptExited_action( PromptObject promptOb_passed )
    {
        Time.timeScale = 1f;
        myGameState = GameState.Unpaused;
    }
