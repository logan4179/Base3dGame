...
public class PV_Environment : PV_Object
{
    ...
    private void Awake()
    {
        LogHistoric($"PV_Environment Awake()", PV_LogFormatting.UserMethod );
        if ( Instance == null )
        {
            Instance = this;
        }
        ...
        LogHistoric($"End of PV_Environment Awake", PV_LogFormatting.UserMethod );

    }

    void Start()
    {
        LogHistoric($"PV_Environment Start()", PV_LogFormatting.UserMethod );

        CalculateAndCacheLayersAndMasks();

        #region AREAS ------------------------------
        if( PV_GameManager.Instance.PlayerScript.pvArea_triggering != null )
        {
            LogHistoric($"playerscript instance was found to have a pre-cahced triggering area: '{PV_GameManager.Instance.PlayerScript.pvArea_triggering.name}'");
			if (PV_GameManager.Instance.PlayerScript.pvArea_triggering != null)
			{
				LogHistoric($"playerscript instance was found to have a pre-cahced triggering area: '{PV_GameManager.Instance.PlayerScript.pvArea_triggering.name}'");
				LAMS_Manager.Instance.InitializeSystem( PV_GameManager.Instance.PlayerScript.pvArea_triggering );
			}
        }
        #endregion

        LogHistoric($"End of PV_Environment Start", PV_LogFormatting.UserMethod );
