...
public class MGR_BugEnemy : PV_Object
{
    ...
    private void Awake()
    {
        LogHistoric( $"mgrBasicEnemy Awake()", PV_Enums.PV_LogFormatting.UnityAPIMethod );
        if( Instance == null )
        {
            Instance = this;
        }
        ...
        LogHistoric( $"mgrBasicEnemy Awake() end", PV_Enums.PV_LogFormatting.UnityAPIMethod );

    }

    void Start()
    {
        LogHistoric( $"mgrBasicEnemy Start()", PV_Enums.PV_LogFormatting.UnityAPIMethod );
        ...
        {
            LogHistoric( $"Had '{bugInstances.Count}' bug instances logged at start." );
        }
        else
        {
        ...
        LogHistoric($"mgrBasicEnemy Start() end.", PV_Enums.PV_LogFormatting.UnityAPIMethod);
    }
    ...
    {
        LogHistoric($"{nameof(ActivateBug)}('{bug_passed.name}')");
    ...
    public void UnregisterAndDeactivateBug( Enemy_Bug bug_passed )
    {
        LogHistoric($"{nameof(UnregisterAndDeactivateBug)}('{bug_passed.name}')");
    ...
    public void HandlePlayerDeath()
    {
        ...
        foreach( Enemy_Bug bug in BugInstances_active )
        {
            ...
            {
                bug.LogHistoric($"Player dead. Reinitializing bug: '{bug.name}'...", PV_Enums.PV_LogFormatting.Standard, true);
