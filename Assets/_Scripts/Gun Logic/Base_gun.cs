...
public abstract class Base_gun : PV_Object
{
    ...
    public bool CanPullTrigger
    {
        get
        {
            ...
            LogHistoric( $"CanPullTrigger returned false. AmReloading: '{AmReloading}'", PV_LogFormatting.Standard, true );
            return false;
        }
    }
	...
	protected void BaseInit()
    {
        ...
        LogHistoric("BaseInit() end");

    }

    public virtual void FireBullet()
    {
        PV_Debug.LogWithConsoleConditional($"FireBullet(), totalAmmunition_inClip: '{count_ammunitionCurrentlyInClip}", amDebuggingScript, PV_LogFormatting.UserMethod);
        ...
        //Ray ray_bullet = new Ray(Camera.main.transform.position + Camera.main.transform.forward * (_scrCamera.dist_follow + 0.8f), Camera.main.transform.position + (Camera.main.transform.forward * 100f));
        ...
        if ( Physics.Raycast(ray_bullet, out rcHit_Bullet, MyStats.dist_BulletRaycast, PV_Environment.Instance.Mask_Shootable) )
        {
            ...
            PV_Debug.LogWithConsoleConditional( $"HIT object: '{rcHit_Bullet.transform.name}', layer: '{lyr}'.", amDebuggingScript );
    ...
    {
		LogHistoric($"Base.InitiateReload(), '{anim_player.GetCurrentAnimatorClipInfo(1)[0].clip.name}'", PV_LogFormatting.UserMethod, amDebuggingScript);
    ...
    {
        LogHistoric($"Base.ReloadMe()", PV_LogFormatting.UserMethod);
    ...
    {
        LogHistoric($"UpdateAmmunitionDisplay(totalAmmunition_inClip: '{count_ammunitionCurrentlyInClip}')", PV_LogFormatting.UserMethod);
        ...
        LogHistoric($"end. totalAmmunition_inClip: '{count_ammunitionCurrentlyInClip}'. txt: '{PV_GameManager.Instance.CanvasScript_inGame._txt_Ammunition.text}'", PV_LogFormatting.Standard );
    }
    /// <summary>Should be called whenever the player selects/equips this gun in-game.</summary>
    public void EquipMe()
    {
        LogHistoric($"Base.EquipMe('{name}'). Finding existing slot with my ammo with itemName of: '{MyStats.itemName_myAmmo}'", PV_LogFormatting.UserMethod);
    ...
    {
        LogHistoric( "Base.BeginDraw()", PV_LogFormatting.UserMethod );
    ...
    {
        LogHistoric($"Base.UnequipMe('{name}')", PV_LogFormatting.UserMethod);
    ...
    {
        LogHistoric($"CycleFireMode()", PV_LogFormatting.UserMethod);
        ...
        LogHistoric($"CycleFireMode() end. firemode now set to: '{MyFireMode}'");
    }
    ...
    {
        LogHistoric($"Base.OnCycleFireMode_action()", PV_LogFormatting.UserMethod);
        AudioSource.PlayClipAtPoint(MyStats.audioClip_switchFireMode, trans_MuzzlePos.position);
        LogHistoric($"fire mode changed to: '{MyFireMode}'");
    }
