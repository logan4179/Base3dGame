...
public class Gun_pistol : Base_gun
{
    void Start()
    {
        LogHistoric( "Gun_pistol Start() begin", PV_LogFormatting.UnityAPIMethod );
        CheckIfKosher();
        BaseInit();
        ...
        LogHistoric( "Gun_pistol start() end", PV_LogFormatting.UnityAPIMethod );
    ...
    {
        LogHistoric( $"FireBullet start", PV_LogFormatting.UserMethod );
        ...
        if ( count_ammunitionCurrentlyInClip > 0 )
        {
            ...
            {
                LogHistoric( $"setting burst alarm to '{fireRate_calculated}'" );
                cd_Burst = fireRate_calculated;
            }
        }

        LogHistoric( $"FireBullet end. Set cd_FireFullAuto to: '{cd_FireFullAuto}', totalAmmunition_inClip: '{count_ammunitionCurrentlyInClip}'", PV_LogFormatting.UserMethod );
    }

    public override void ConstrainToHand()
    {
        LogHistoric( $"ConstrainToHand('{name}')", PV_LogFormatting.UserMethod );
        anim.SetBool( animID_WeaponDrawn, true );
    }

    public override void ConstrainToHolster()
    {
        LogHistoric( $"ConstrainToHolster('{name}')", PV_LogFormatting.UserMethod );
        anim.SetBool( animID_WeaponDrawn, false );
    }

    public override void BeginDraw()
    {
        LogHistoric( "BeginDraw()", PV_LogFormatting.UserMethod );
    ...
    {
        LogHistoric( "SetMeToDrawn()", PV_LogFormatting.UserMethod) ;
